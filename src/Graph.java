// Java implementation of Kosaraju's algorithm to print all SCCs
import java.io.*;

import java.util.Iterator;
//import java.util.Stack;
//import java.util.LinkedList;


// This class represents a directed graph using adjacency list
// representation


class Graph
{
    private int V;   // No. of vertices
    private MyLinkedList3[] adj; //Adjacency List
 
    //Constructor
    Graph(int v)
    {
        V = v;
        adj = new MyLinkedList3[v + 1];
        for (int i=1; i < v + 1; ++i)
            adj[i] = new MyLinkedList3();
    }
 
    //Function to add an edge into the graph
    void addEdge(int v, int w)  { adj[v].addFirst(w); }
 
    // A recursive function to print DFS starting from v
    void DFSUtil(int v,boolean visited[]) throws IOException
    {
        // Mark the current node as visited and print it
        visited[v] = true;
        FW f = new FW();
        f.CreateAndWriteToFile(Integer.toString(v));
       
 
        int n;
 
        // Recur for all the vertices adjacent to this vertex        
        
        Iterator<Integer> i =adj[v].iterator();
        while (i.hasNext())
        {
            n = i.next();
            if (!visited[n])
                DFSUtil(n,visited);
        }        
        
    }
 
    // Function that returns reverse (or transpose) of this graph
    Graph getTranspose()
    {
        Graph g = new Graph(V);
        for (int v = 1; v <= V; v++)
        {
            // Recur for all the vertices adjacent to this vertex
            Iterator<Integer> i =adj[v].iterator();
            while(i.hasNext())
                g.adj[i.next()].addFirst(v);
        }
        return g;
    }
 
    void fillOrder(int v, boolean visited[], MyStack stack)
    {
        // Mark the current node as visited and print it
        visited[v] = true;
 
        // Recur for all the vertices adjacent to this vertex
        Iterator<Integer> i = adj[v].iterator();
        while (i.hasNext())
        {
            int n = i.next();
            if(!visited[n])
                fillOrder(n, visited, stack);
        }
 
        // All vertices reachable from v are processed by now,
        // push v to Stack
        stack.push(new Integer(v));
    }
 
    // The main function that finds and prints all strongly
    // connected components
    void printSCCs() throws IOException
    {
        MyStack stack = new MyStack();
 
        // Mark all the vertices as not visited (For first DFS)
        boolean visited[] = new boolean[V + 1];
        for(int i = 1; i < V + 1; i++)
            visited[i] = false;
 
        // Fill vertices in stack according to their finishing
        // times
        for (int i = 1; i <= V; i++)
            if (visited[i] == false)
                fillOrder(i, visited, stack);
 
        // Create a reversed graph
        Graph gr = getTranspose();
 
        // Mark all the vertices as not visited (For second DFS)
        for (int i = 1; i <= V; i++)
            visited[i] = false;
 
        // Now process all vertices in order defined by Stack
        while (stack.isEmpty() == false)
        {
            // Pop a vertex from stack
            int v = (int)stack.pop();
            
            
 
            // Print Strongly connected component of the popped vertex
            if (visited[v] == false)
            {
                gr.DFSUtil(v, visited);
                FW f = new FW();
                f.CreateAndWriteToFile("Component");
            }
        }
    }
     
 
    // Driver method
    public static void main(String args[]) throws IOException
    {
    	// read the text file; returns an array containing the numbers in the text file
    	readFile r = new readFile();
    	// copy the data in dataArray
       	int[] dataArray = r.readFile("C:/Users/User/Desktop/vertices.txt");
       	int sizeOfDataArray = r.readFile("C:/Users/User/Desktop/vertices.txt").length;      	
       	
       	//System.out.println(sizeOfDataArray);
    	
        // Create a graph given in the above diagram
        Graph g = new Graph(dataArray[0]);
        
        for (int i = 2; i < sizeOfDataArray - 1; i++)
        {
        	if (i % 2 == 0)
        	{
        		g.addEdge(dataArray[i], dataArray[i + 1]);
        	}        	
        }
        
        /*
        for (int i = 1; i <= dataArray[0]; i++)
        {        	
        		System.out.println("Neighbours of " + i + " are" + g.adj[i]);        	
        }
        */    
 
        
        FW f = new FW();
        f.CreateAndWriteToFile("Component");
        g.printSCCs();
        
                
      
      
      
      
      
      
      
       
              
        
       // r.closeFile();
    }
}